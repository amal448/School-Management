...
import { buildTopperModule } from './topper'
import { buildStatsModule } from './stats'

import { Router, RequestHandler, ErrorRequestHandler } from 'express'
import { buildAnnouncementModule } from './announcement'
import { createUploadRouter } from 'src/interfaces/routes/upload.routes'
...
export function buildDependencies(): AppDependencies {
  ...
  const topper = buildTopperModule(authMW)
  const announcement = buildAnnouncementModule(logger, authMW)
  const stats = buildStatsModule(authMW)
  return {
  uploadRouter: createUploadRouter(authMW),
