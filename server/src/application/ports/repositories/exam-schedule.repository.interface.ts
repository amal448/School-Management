...
export interface IExamScheduleRepository {
  ...
  findByTeacherAndClass(teacherId: string, classId: string): Promise<ExamScheduleEntity[]>
  findByTeacherIdAndStatuses(teacherId: string,statuses: MarksStatus[]): Promise<ExamScheduleEntity[]>
  findByExamAndClass(examId: string,classId: string): Promise<ExamScheduleEntity[]>
