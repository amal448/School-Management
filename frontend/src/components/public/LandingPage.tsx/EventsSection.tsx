                      ...
                      <CardHeader className="pb-3 flex flex-row items-start justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <Badge variant="outline" className="bg-white text-xs">{ann.type}</Badge>
                            <span className="text-xs text-slate-500 font-medium flex items-center gap-1">
                              <Calendar className="w-3 h-3" /> {ann.date}
                            </span>
                          </div>
                          <CardTitle className="text-xl leading-tight group-hover:text-primary transition-colors">{ann.title}</CardTitle>
                        </div>
                        {ann.isNew && (
                          <Badge className="bg-primary hover:bg-primary/90 text-[10px] uppercase font-bold shrink-0">New</Badge>
                        )}
                      </CardHeader>
                      <CardContent>
                        <CardDescription className="text-slate-600 text-sm leading-relaxed">{ann.excerpt}</CardDescription>
                      </CardContent>
                    </Card>
                  </motion.div>
                ))}
                  ...
                  {UPCOMING_EVENTS.map((evt, i) => (
                    ...
                    >
                      ...
                      <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl w-16 h-16 flex flex-col items-center justify-center shrink-0 group-hover:bg-primary transition-colors duration-300">
                        <span className="text-white text-xs font-semibold uppercase">{evt.month}</span>
                        <span className="text-white text-xl font-bold leading-none mt-0.5">{evt.date}</span>
                      </div>

                      {/* Content */}
                      <div>
                        <h4 className="text-white font-medium group-hover:text-primary-foreground transition-colors mb-2">{evt.title}</h4>
